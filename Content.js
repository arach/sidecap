(function () {
  ...
  function renderHistory() {
    ...
    if (!config.historyEnabled) {
      historyEmpty.textContent = "History is disabled. Enable it in Settings.";
  ...
  function mount() {
    ...
    historyLabel.textContent = "Caption history";
    ...
    // Assemble all tabs into body
    body.append(captionsTab, historyTab, dictionaryTab, settingsTab);

    // Assemble devtools
    devtools.append(header, body);

    // Add devtools to root
    ...
    function setDictStatus(message) {
      if (message) {
        ...
        dictStatus.classList.remove("is-hidden");
      } else {
        dictStatus.classList.add("is-hidden");
      }
    }

    function clearDictValues() {
      defValue.textContent = "";
      transValue.textContent = "";
      pronValue.textContent = "";
      formsValue.textContent = "";
    ...
    function renderDictionaryData(parsed) {
      ...
      dictMeta.textContent = metaParts.join(" | ");
    ...
    async function lookupWord(wordText) {
      ...
      try {
        ...
        clearDictValues();
        setDictStatus("Dictionary lookup failed.");
      }
    }
    ...
    state.dockEl = devtools;

    // Tab switching
    const tabButtons = [captionsTabBtn, historyTabBtn, dictionaryTabBtn, settingsTabBtn];
    const tabPanels = [captionsTab, historyTab, dictionaryTab, settingsTab];
    tabButtons.forEach((button) => {
      button.addEventListener("click", () => {
        switchTab(button.dataset.tab, tabPanels, tabButtons);
      });
    });

    // Collapse/expand devtools
    collapseButton.addEventListener("click", () => {
      state.devToolsCollapsed = !state.devToolsCollapsed;
      devtools.classList.toggle("is-collapsed", state.devToolsCollapsed);
      collapseButton.textContent = state.devToolsCollapsed ? "+" : "−";
    });

    // Keyboard shortcut (Alt+C to toggle devtools)
    ...
    // Word click in captions - show dictionary and replay
    overlay.addEventListener("click", (event) => {
      ...
      clearDictValues();
      dictWord.textContent = wordText;
      dictMeta.textContent = "Loading...";
      setDictStatus("Loading dictionary...");
      lookupWord(wordText);

      // Switch to dictionary tab
      switchTab("dictionary", tabPanels, tabButtons);
    ...
    select.addEventListener("change", () => {
      ...
      persistConfig();
      if (state.activeWord && state.activeTab === "dictionary") {
